  ...
  return (
    <div className="relative w-24 h-24 flex items-center justify-center shrink-0">
      ...
      <div className="absolute flex flex-col items-center leading-tight">
        ...
        <span className="text-[10px] text-gray-400 dark:text-gray-500 dark:text-gray-500 dark:text-gray-500 dark:text-gray-500">
          / 100
        </span>
      </div>
    </div>
      ...
      {items.map((item, i) => (
        ...
        >
          <button
            ...
            className="w-full flex items-start justify-between px-4 py-3.5 text-left hover:bg-gray-50 dark:bg-[#1a1a1a] dark:hover:bg-[#1a1a1a] transition-colors gap-3"
          >
            ...
            <ChevronDown
              size={15}
              className={`text-gray-400 dark:text-gray-500 dark:text-gray-500 dark:text-gray-500 dark:text-gray-500 shrink-0 mt-0.5 transition-transform ${openIdx === i ? "rotate-180" : ""}`}
            />
          </button>
          {openIdx === i && (
            <div className="px-4 pb-4 border-t border-gray-100 dark:border-[#2a2a2a] bg-gray-50 dark:bg-[#1a1a1a]/50 dark:bg-[#1a1a1a]/50">
              <p className="mt-3 mb-1 text-[11px] font-semibold text-[#ea580c] uppercase tracking-wider">
                Why they ask this
              </p>
              <p className="text-sm text-gray-500 dark:text-gray-400 dark:text-gray-600 dark:text-gray-400 mb-3 leading-relaxed">
                {item.intention}
              </p>
              <p className="mb-1 text-[11px] font-semibold text-[#ea580c] uppercase tracking-wider">
                How to answer
              </p>
              <p className="text-sm text-gray-500 dark:text-gray-400 dark:text-gray-600 dark:text-gray-400 leading-relaxed">
                {item.answer}
              </p>
            </div>
          )}
...
const Skeleton = () => (
  <div className="flex flex-col gap-4 animate-pulse">
    <div className="bg-white dark:bg-[#121212] border border-gray-200 dark:border-[#222] rounded-2xl p-6">
      ...
      <div className="h-4 bg-gray-100 dark:bg-[#1e1e1e] rounded w-1/4 mb-6" />
    ...
    {[1, 2].map((k) => (
      ...
      >
        ...
        {[1, 2, 3].map((j) => (
          <div
            key={j}
            className="h-12 bg-gray-100 dark:bg-[#1e1e1e] rounded-xl mb-2"
          />
        ))}
      </div>
    ))}
...
const GenerateReportForm = ({ session, onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (resumeMode === MODES.upload && !resumeFile) {
      toast.error("Please upload your resume PDF");
      return;
    }
    if (resumeMode === MODES.text && !resumeText.trim()) {
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-5">
      ...
      <div className="bg-white dark:bg-[#121212] border border-gray-200 dark:border-[#222] rounded-2xl p-5 shadow-sm">
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-400 dark:text-gray-500 dark:text-gray-500 dark:text-gray-500 dark:text-gray-500 mt-0.5 line-clamp-2">
      ...
      <div className="bg-white dark:bg-[#121212] border border-gray-200 dark:border-[#222] rounded-2xl p-5 shadow-sm">
        ...
        {/* Mode toggle */}
        <div className="flex gap-2 mb-4 p-1 bg-gray-100 dark:bg-[#1e1e1e] rounded-xl w-fit">
            ...
            <button
              ...
              className={`px-4 py-1.5 rounded-lg text-xs font-medium transition-all ${
                resumeMode === m
                  ...
                  : "text-gray-400 dark:text-gray-500 dark:text-gray-500 dark:text-gray-500 hover:text-gray-700"
              }`}
            >
        ...
        {resumeMode === MODES.upload ? (
          resumeFile ? (
            ...
            >
              <Upload
                size={20}
                className="text-gray-400 dark:text-gray-500 dark:text-gray-500 dark:text-gray-500 dark:text-gray-500 mb-2"
              />
              <p className="text-sm text-gray-400 dark:text-gray-500 dark:text-gray-500 dark:text-gray-500">
                Drop your PDF here or{" "}
                <span className="text-[#ea580c] font-medium">browse</span>
              </p>
              <p className="text-xs text-gray-400 dark:text-gray-500 dark:text-gray-500 dark:text-gray-500 dark:text-gray-500 mt-1">
                PDF only · max 3 MB
              </p>
              <input
                ref={fileRef}
...
const ReportDetailView = () => {
  ...
  const handleDownloadPdf = async () => {
    ...
    setPdfLoading(true);
    try {
      await generatePdf(report._id, session?.title);
    } catch {
    ...
    return (
      <div className="flex flex-col gap-4">
        <button
          ...
          className="flex items-center gap-1.5 text-sm text-gray-400 dark:text-gray-500 dark:text-gray-500 dark:text-gray-500 hover:text-gray-900 dark:text-gray-100 transition-colors w-fit"
        >
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      {/* Technical Questions */}
      <Section
        title={`Technical Questions (${report.technicalQuestions?.length ?? 0})`}
      >
      ...
      <Section
        title={`Behavioural Questions (${report.behavioralQuestions?.length ?? 0})`}
      >
      ...
      {/* Preparation Plan */}
      <Section
        title={`Preparation Plan (${report.preparationPlan?.length ?? 0} days)`}
      >
              ...
              >
                ...
                <ul className="flex flex-col gap-1.5">
                  {day.tasks.map((task, ti) => (
                    <li
                      key={ti}
                      className="flex items-start gap-2 text-xs text-gray-500 dark:text-gray-400 dark:text-gray-600 dark:text-gray-400 leading-relaxed"
                    >
      ...
      <div className="bg-white dark:bg-[#121212] border border-gray-200 dark:border-[#222] rounded-2xl p-5 shadow-sm">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-0.5 text-xs text-gray-400 dark:text-gray-500 dark:text-gray-500 dark:text-gray-500 dark:text-gray-500">
      ...
      <div className="flex justify-center pb-4">
        <Link
          to="/dashboard/new"
          className="flex items-center gap-2 text-sm text-gray-400 dark:text-gray-500 dark:text-gray-500 dark:text-gray-500 dark:text-gray-500 hover:text-[#ea580c] transition-colors"
        >
